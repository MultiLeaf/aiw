...
The package distributed by AI Workflow is a catalog, not an automatic project install. `aiw install` writes only the target-specific `ai-init` bootstrap and neutral state. `/ai-init` runs CLI commands through `npx --yes --package=@multileaf/ai-workflow -- aiw <command>` because installation does not add a global `aiw` binary or a dependency to the consuming project. It scans the project, recommends matching capabilities, asks which to activate, and syncs only the selected resources. Supported rules and project-quality guidance are personalized from detected project tools; existing or user-edited files are preserved and reported as conflicts.
