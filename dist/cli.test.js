...
describe("AI Workflow CLI", () => {
    it("installs the neutral structure and the selected Codex target", async () => {
        ...
        await expect(readFile(join(cwd, ".agents/skills/ai-init/SKILL.md"), "utf8")).resolves.toContain("All generated AI Workflow artifacts must be written in English");
    });
    it("scans a JavaScript project and writes its profile", async () => {
        ...
        await expect(readFile(join(cwd, ".aiw/profile.yml"), "utf8")).resolves.toContain("languages: [typescript]");
        await expect(readFile(join(cwd, ".aiw/profile.yml"), "utf8")).resolves.toContain("key: runtime.language");
        await expect(readFile(join(cwd, ".aiw/profile.yml"), "utf8")).resolves.toContain("method: deterministic");
        await expect(readFile(join(cwd, ".aiw/profile.yml"), "utf8")).resolves.toContain('evidence: ["src/main.ts"]');
    });
    ...
    it("migrates the generated target and updates the manifest", async () => {
        ...
        await expect(readFile(join(cwd, ".aiw/manifest.yml"), "utf8")).resolves.toContain("active: claude");
    });
    ...
    it("persists an accepted fact override", async () => {
        ...
        expect(result.exitCode).toBe(0);
        await expect(readFile(join(cwd, ".aiw/overrides.yml"), "utf8")).resolves.toContain("action: accept");
    });
    it("persists an edited fact override", async () => {
        const cwd = await project();
        await run(["install"], cwd);
        await run(["confirm", "--edit", "package-manager=pnpm"], cwd);
        await expect(readFile(join(cwd, ".aiw/overrides.yml"), "utf8")).resolves.toContain("value: pnpm");
    });
    ...
    it("generates and selects recommendations through the CLI", async () => {
        ...
        await run(["install"], cwd);
        const { writeFile } = await import("node:fs/promises");
        await writeFile(join(cwd, "package.json"), JSON.stringify({ devDependencies: { vitest: "latest", typescript: "latest" } }));
    ...
    it("generates project-specific resources through the CLI", async () => {
        ...
        await expect(stat(join(cwd, ".aiw/generated/rules/tdd-project-policy.md"))).resolves.toBeTruthy();
    });
    it("preserves manually changed generated resources", async () => {
        const cwd = await project();
        await run(["install"], cwd);
        const { writeFile } = await import("node:fs/promises");
        await writeFile(join(cwd, "package.json"), JSON.stringify({ devDependencies: { typescript: "latest" } }));
        await mkdir(join(cwd, "src"));
        await writeFile(join(cwd, "src", "main.ts"), "export {};\n");
